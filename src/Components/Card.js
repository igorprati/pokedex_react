              ...
              loading ? <span className="app__list__card__category__button"></span> :
                ...
                pokemon.types.map((item) => (
                  <Link to={'/type/' + item.type.url.split('/')[6]}>
                    <span props={item.type.name} key={item.type.name} className={'app__list__card__category__button app__list__card__category__button--' + item.type.name}>{primeiraLetra(item.type.name)}</span>
                  </Link>
                ))
            }
          </div>
